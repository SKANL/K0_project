...
export type IntegrationProviderPort = Readonly<{
  provider: IntegrationProvider; toolkitVersion: string;
...
export function createProviderSecretPort(config: Readonly<Partial<Record<IntegrationProvider, string | undefined>>>): ProviderSecretPort {
  const secrets = new Map(Object.entries(config).filter((entry): entry is [IntegrationProvider, string] => typeof entry[1] === "string" && entry[1].trim().length > 0));
  return Object.freeze({ getSecret: (provider) => secrets.get(provider) });
}
