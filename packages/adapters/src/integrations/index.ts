...
function integrationCapability(provider: IntegrationProvider, capabilities: readonly string[], limits: Readonly<Record<string, number>>): IntegrationCapabilityContract { return Object.freeze({ version: "integration-capability/v1", capabilities: Object.freeze([...capabilities]), limits: Object.freeze({ ...limits }), credentialReference: `vault://${provider}`, health: async () => ({ healthy: true }) }); }
export function createSendblueAdapter(host: { connect: (input: { tenantId: string; userId: string; scopes: readonly string[]; authorizationCode: string }) => Promise<string>; lookupDestinationCapability: (destination: string) => Promise<{ capable: boolean }>; send: (input: { destination: string; idempotencyKey: string }) => Promise<{ state: "sent" | "delivered" | "error"; costMicros: number; latencyMs: number; providerDeliveryId?: string }>; status: (input: { idempotencyKey: string; providerDeliveryId?: string }) => Promise<{ state: "sent" | "delivered" | "error" | "unknown"; providerDeliveryId?: string }> }): IntegrationProviderPort { return { provider: "sendblue", toolkitVersion: "api", capability: integrationCapability("sendblue", ["message.send", "message.reconcile"], { maxDestinationLength: 16, maxRequestsPerMinute: 60 }), connect: async (input) => ({ externalAccountId: await host.connect(input) }), execute: async (input) => { const destination = input.metadata.destination ?? ""; if (!e164.test(destination) || !(await host.lookupDestinationCapability(destination)).capable) return { state: "error", costMicros: 0, latencyMs: 0 }; return host.send({ destination, idempotencyKey: input.idempotencyKey }); }, reconcile: async (input) => host.status(input) }; }
export function createComposioAdapter(host: { toolkitVersion: string; connect: (input: { tenantId: string; userId: string; scopes: readonly string[]; authorizationCode: string }) => Promise<string>; execute: (input: { operation: string; idempotencyKey: string; metadata: Readonly<Record<string, string>> }) => Promise<{ state: "sent" | "delivered" | "error"; costMicros: number; latencyMs: number }>; reconcile: (input: { idempotencyKey: string; providerDeliveryId?: string }) => Promise<{ state: "sent" | "delivered" | "error"; providerDeliveryId?: string }> }): IntegrationProviderPort { if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(host.toolkitVersion)) throw new Error("COMPOSIO_TOOLKIT_VERSION_INVALID"); return { provider: "composio", toolkitVersion: host.toolkitVersion, capability: integrationCapability("composio", ["tool.execute", "tool.reconcile"], { maxMetadataEntries: 32, maxRequestsPerMinute: 60 }), connect: async (input) => ({ externalAccountId: await host.connect(input) }), execute: async (input) => host.execute({ operation: input.operation, idempotencyKey: input.idempotencyKey, metadata: input.metadata }), reconcile: async (input) => host.reconcile(input) }; }
export function createAppleAdapter(): IntegrationProviderPort { return { provider: "apple", toolkitVersion: "capability-matrix/v1", capability: integrationCapability("apple", ["notes", "shortcuts"], { maxRequestsPerMinute: 30, maxPayloadChars: 16_384 }), connect: async () => ({ externalAccountId: "local-consent" }), execute: async () => ({ state: "error", costMicros: 0, latencyMs: 0 }), reconcile: async () => ({ state: "unknown" }) }; }
